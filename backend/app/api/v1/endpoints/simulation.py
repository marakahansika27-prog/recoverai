...
def run_batch_simulation(
    ...
    # Record benchmark run in DB with safe exception handling
    try:
        BenchmarkRun.metadata.create_all(session.bind)
        bench = BenchmarkRun(
        ...
        session.rollback()
        print(f"[BENCHMARK DB WARNING] Non-fatal DB recording notice: {e}")
...
def get_latest_benchmark(session: Session = Depends(get_session)):
    try:
        ...
        print(f"[BENCHMARK LATEST WARNING] Could not fetch latest run: {e}")
        return None
